...
### Suggestion Engine (AI-207)

The Python server includes a lightweight v1 suggestion engine in `python/tools/suggestion_engine.py`.
It recommends relevant tools from query text using keyword + basic scoring (no heavy ML dependency).

Example:

```text
Query: "show loan details"
Suggested tools: ["get_loan_details", "create_loan", "get_loan"]
```
